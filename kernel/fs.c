...
void delete_helper(struct inode *ip, uint *addrs, int index, int depth) {
    int j;
    struct buf *bp;
    uint *a;
    bp = bread(ip->dev, addrs[index]);
    a = (uint *)bp->data;
    for (j = 0; j < NINDIRECT; j++) {
        if (a[j]) {
            if (depth < 1) {
                delete_helper(ip, a, j, depth + 1);
            }
            bfree(ip->dev, a[j]);
        }
    }
    brelse(bp);
    // bfree(ip->dev, addrs[index]);
    // addrs[index] = 0;
}
...
void itrunc(struct inode *ip) {
    ...
    if (ip->addrs[NDIRECT + 1]) {
        delete_helper(ip, ip->addrs, NDIRECT + 1, 0);
